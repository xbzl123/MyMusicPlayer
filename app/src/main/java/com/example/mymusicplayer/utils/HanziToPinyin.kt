...
class HanziToPinyin constructor(hasChinaCollator: Boolean){

    private val TAG = "HanziToPinyin"
    ...
    private var mHasChinaCollator = hasChinaCollator
    ...
    companion object {
        ...
        synchronized(HanziToPinyin::class.java) {
            var sInstance:HanziToPinyin? = null
            if (sInstance != null) {
                return sInstance
            }
            ...
            for (i in locale.indices) {
                if (locale[i].language.contains("zh")) {
    ...
    private fun doSelfValidation(): Boolean {
        ...
        for (c in UNIHANS) {
            ...
            if (cmp >= 0) {
                Log.e(
                    TAG, "Internal error in Unihan table. " + "The last string \"" + lastString
    ...
    private fun getToken(character: Char): Token {
        ...
        var j = 0
        val byte:Byte = 0
        while (j < PINYINS[offset].size && !PINYINS[offset][j].equals(byte)) {
